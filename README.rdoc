...
The app includes icons for each team. These icons are located in the Logos folder in apps root. An edit page to add logos has been included, but is not linked from inside the app. Each teams edit page can be accessed directly by typing 'http://localhost:3000/teams/1/edit, replacing the team number for each.
...
To import the data, open a terminal session, navigate to the 'app' directory:
...
To complete the draft, open a terminal session, navigate to the 'app' directory:

1) From the command prompt, type 'rake draft:all'

2) Restart the app by typing 'rails server'

To reset the draft, open a terminal session, navigate to the 'app' directory:
